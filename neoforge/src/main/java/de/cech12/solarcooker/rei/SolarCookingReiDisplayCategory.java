...
public class SolarCookingReiDisplayCategory implements DisplayCategory<SolarCookingReiDisplay> {
    ...
    public List<Widget> setupDisplay(SolarCookingReiDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createLabel(new Point(bounds.x + bounds.width - 5, bounds.y + 5), Component.translatable("category.rei.cooking.time&xp", df.format(display.getXp()), df.format(cookingTime / 20.0))).noShadow().rightAligned().color(-12566464, -4473925));
